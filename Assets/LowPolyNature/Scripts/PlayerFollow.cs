...
public class PlayerFollow : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        if (IsRotateActive)
        {
            ...
            if (_invert == true )
            {
                 h = (Input.GetAxis("Mouse X") * -1) * RotationsSpeed;
                 v = (Input.GetAxis("Mouse Y") * -1) * RotationsSpeed;
            }
            else
            {
                 h = Input.GetAxis("Mouse X") * RotationsSpeed;
                 v = Input.GetAxis("Mouse Y") * RotationsSpeed;
            }
            ...
            Quaternion camTurnAngle = Quaternion.AngleAxis(h, Vector3.down);
